...
/*   Updated: 2019/02/22 20:20:36 by elchrist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < 3)
	{
		...
		if (str_counter == 4)
		{
        	...
        	while (new_i < 3)
       		...
       		ft_strncpy((*tetri).shape[3], "....", 5); // not sure about this one, to be tested
		}
      i++;
    }
}
...
{
	if ((tetri->shape)[0][2] == tetri->letter && (tetri->shape[1][0] == tetri->letter) && (tetri->shape[1][1] == tetri->letter) && (tetri->shape[1][2] == tetri->letter))
		return (1);
	return (0);
}
...
int tetri_drawing(size_t y, size_t x, char **map, t_tetr *tetri, int map_size)
{
  ...
  i = 0;
	...
	while (j < (*tetri).height && (y + j) < map_size)
	{
		...
		{
			k++;
		// 	if ((tetri->shape)[0][1] == '.' && k > 1)
		// 		k++;
			if (stupid_func(tetri))
				k++;
		}
		//printf("%d - eto vyvod stupid_func\n", stupid_func(tetri));
		while (i < (*tetri).width && (x + i - k) <= map_size) // если здесь добавить -k, то отрисовка нормальная
		{
